...
import {
  ...
  Moon, Sun, UserPlus, ChevronRight,
} from "lucide-react";
  ...
  return (
    <Link href={item.href} onClick={onClick}>
      ...
      >
        ...
        <item.icon className={cn("shrink-0 transition-transform", active ? "h-5 w-5" : "h-5 w-5 group-hover:scale-110")} />
        {!collapsed && (
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      <aside className="hidden md:flex w-60 flex-col fixed inset-y-0 z-50 bg-sidebar border-r border-white/10 shadow-xl shadow-black/20">
        ...
        {/* Nav label */}
        <p className="px-5 text-[10px] font-semibold uppercase tracking-widest text-white/30 mb-1">Navigation</p>
        ...
        <div className="p-4 flex items-center justify-between gap-3">
          <Link href="/members/new">
            <Button
              size="sm"
              className="bg-[hsl(43,90%,50%)] hover:bg-[hsl(43,90%,44%)] text-[hsl(142,60%,15%)] font-semibold h-8 text-xs gap-1.5 shadow-none rounded-lg flex-1"
            >
      ...
      <div className="flex-1 md:ml-60 flex flex-col min-h-screen">
        ...
        {/* Mobile Top Header */}
        <header className="md:hidden sticky top-0 z-40 bg-background/95 backdrop-blur-sm border-b border-border flex items-center justify-between px-4 h-14 gap-3">
        ...
        {/* Mobile Bottom Nav */}
        <nav className="md:hidden fixed bottom-0 inset-x-0 z-50 bg-background/95 backdrop-blur-sm border-t border-border safe-area-pb">
