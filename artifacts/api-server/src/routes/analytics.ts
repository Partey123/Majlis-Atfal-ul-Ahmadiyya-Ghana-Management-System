...
router.get("/analytics/by-sector", async (_req, res) => {
  const sectors = ["Northern Sector", "Middle Sector", "Southern Sector"];
  const results = await Promise.all(
    sectors.map(async (sector) => {
      const [counts] = await db.select({
        ...
        kabir: sql<number>`count(*) filter (where wing = 'atfal_kabir')`,
        khuddam: sql<number>`count(*) filter (where wing = 'khuddam')`,
      }).from(membersTable).where(eq(membersTable.sector, sector));

      return {
        sector,
        totalMembers: Number(counts.total),
        atfalSughir: Number(counts.sughir),
        atfalKabir: Number(counts.kabir),
        khuddam: Number(counts.khuddam),
      };
    })
  );
  res.json(results);
