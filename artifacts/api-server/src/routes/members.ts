...
router.post("/members", async (req, res) => {
  ...
  // Increment circuit/jamaat usage counts
  await db.execute(sql`UPDATE circuits SET usage_count = usage_count + 1 WHERE name = ${data.circuit}`);
  await db.execute(sql`UPDATE jamaats SET usage_count = usage_count + 1 WHERE name = ${data.jamaat}`);
